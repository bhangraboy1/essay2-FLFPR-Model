...
* Merging Block 5 with Block 4
* use "${datapath}Block-5-members-time-disposition-records"
...
* DROP VARIABLES NOT NEEDED FOR TABLES or REGRESSIONS
rename v2  q29
rename v3  q209
rename v4  q210
rename v5  q211
...
* Generate Weights
replace weight = MLT/100 if ((NSS==NSC) & ((year==2009) | (year==2011)))
replace weight = MLT/200 if ((NSS!=NSC) & ((year==2009) | (year==2011)))
...
quietly tabulate Usual_Principal_Activity_Status
* 11 - self employed
* 21 - unpaid work
* 31 - regular employee
* 41 - casual worker
...
preserve
	...
	twoway	(rarea upper lower age_bin, fcolor(gs14) lcolor(none)) ///
		...
		xtitle("Age Group") ///
		ytitle("FLFPR(%)") ///
		legend(order(2 3 4 1) label (1 "95% CI(2011)") label(2 "2007") label(3 "2009") label(4 "2011"))
...
levelsof year, local(years)
foreach y in `years' {
	estpost summarize age HH_Size sg_*  rel_* educ_illiterate educ_literate educ_primary educ_middle educ_secondary educ_graduate work_* [aweight=weight]  if year==`y'
	estimate store year_`y'
}
